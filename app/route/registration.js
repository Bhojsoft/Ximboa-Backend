...
router.post(
  "/",
  uploadProfileImage.single("trainer_image"),
  async function (req, res) {
    ...
    const trainer_image = req.file ? req.file.location : "";
    ...
    try {
      ...
      if (existingUser) {
        return res
          .status(409)
          .json(new ApiError(409, "email_id already exists"));
      }
      ...
      res.status(200).json({
        token,
        profile_image: result.trainer_image || "",
      });
    } catch (err) {
      console.log(err);
      res
